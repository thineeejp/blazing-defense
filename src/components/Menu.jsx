...
export default function Menu({ missions, onStartBattle, onShowHighScores, isFirstLaunch }) {
    ...
    <GameBackground className="flex flex-col items-center justify-center p-4">
      ...
      {showContent && (
        <>
          ...
          {/* Mission Select Grid */}
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 w-full max-w-5xl px-4 animate-slideUpFade delay-200">
            {missions.map((m) => {
          ...
          {/* HIGH SCORESボタン */}
          <div className="mt-8 animate-slideUpFade delay-300">
            <button
              onClick={onShowHighScores}
              className="px-8 py-4 bg-gradient-to-r from-yellow-600/30 to-yellow-500/30 hover:from-yellow-600/50 hover:to-yellow-500/50 border border-yellow-500/50 rounded-xl text-yellow-300 font-bold text-lg transition-all flex items-center gap-3 mx-auto shadow-lg hover:shadow-yellow-500/30 hover:-translate-y-1"
            >
              <Trophy size={24} />
